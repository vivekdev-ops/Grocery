                  ...
                  {isCartEmpty ? (
                    ...
                    <div className="space-y-2">
                      {cart.map(item => (
                        ...
                        >
                          {item.image ? (
                            <img src={item.image} alt={item.title} className="w-12 h-12 rounded-xl object-cover bg-stone-100 shrink-0 border border-stone-100" />
                          ) : (
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="font-bold text-[11px] text-stone-900 truncate leading-tight">{item.title}</p>
                            <p className="text-[10px] text-stone-400 mt-0.5">₹{Number(item.price || 0).toFixed(2)} each</p>
                          </div>
                          <div className="flex items-center gap-1 bg-stone-100 rounded-xl p-0.5 shrink-0">
                            <button onClick={() => updateQuantity(item.cartItemId, -1)} className="w-6 h-6 hover:bg-white rounded-lg flex items-center justify-center text-stone-600 transition cursor-pointer">
                              <Minus size={11} />
                            </button>
                            <span className="text-xs font-black w-5 text-center text-stone-900">{item.quantity}</span>
                            <button onClick={() => updateQuantity(item.cartItemId, 1)} className="w-6 h-6 hover:bg-white rounded-lg flex items-center justify-center text-stone-600 transition cursor-pointer">
                              <Plus size={11} />
                            </button>
                          </div>
