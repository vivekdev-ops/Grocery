      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className="fixed right-0 top-0 bottom-0 w-full max-w-md bg-stone-50 shadow-2xl z-[100] flex flex-col font-sans text-xs"
          ...
          >
            ...
            <div className="h-14 flex items-center justify-between px-4 border-b border-stone-200/80 bg-white shrink-0">
              <div className="flex items-center gap-2.5">
                ...
                <h2 className="font-black text-stone-900 text-base tracking-tight">Cart</h2>
              </div>
              <span className="text-[11px] font-bold text-stone-500">{totalItemsCount || 0} items</span>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-3.5 space-y-3.5">
              ...
              <div className="bg-white rounded-3xl p-4 border border-stone-200/80 shadow-2xs space-y-2">
                <div className="flex items-center justify-between">
                  <span className="font-black text-stone-900 text-xs">Delivering to Home</span>
                </div>
                <div className="text-stone-600 text-xs font-medium flex items-center justify-between">
                ...
                {showAddAddressBox && (
                  <form
                    ...
                    className="bg-stone-50 p-3 rounded-2xl border border-emerald-200 space-y-2 shadow-xs mt-2"
                  >
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <input
                        ...
                        required
                        className="border rounded-xl px-2.5 py-1.5 text-[11px] bg-white"
              ...
              {/* CART ITEMS LIST */}
              {isCartEmpty ? (
                <div className="bg-white rounded-3xl p-8 text-center border border-stone-200/80 shadow-2xs space-y-2">
                  ...
                  <p className="text-[11px] text-stone-400">Add items from the store to get started.</p>
                </div>
              ) : (
                  ...
                  {safeCart.map((item) => {
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-3 min-w-0">
                          {itemImage ? (
                            <img
                              src={itemImage}
                              alt=""
                              className="w-12 h-12 rounded-2xl object-contain bg-stone-50 shrink-0 border border-stone-100 p-1"
                            />
                          ) : (
                        ...
                        {/* QUANTITY CONTROLS */}
                        <div className="flex items-center bg-emerald-600 text-white rounded-xl overflow-hidden shadow-xs h-8 shrink-0">
                          <button
                            type="button"
                            onClick={() => updateQuantity(uniqueKey, -1)}
                            disabled={isOutOfStock}
                            className="px-2.5 h-full hover:bg-emerald-700 font-bold text-xs flex items-center justify-center cursor-pointer"
                          >
                            -
                          </button>
              ...
              {!isCartEmpty && session && (
                <div className="bg-white rounded-3xl p-4 border border-stone-200/80 shadow-2xs space-y-3">
                  <div className="flex items-center justify-between">
                    <h3 className="font-black text-stone-900 text-xs flex items-center gap-1.5">
                      <Tag size={14} className="text-emerald-600" /> Apply Coupon
                    </h3>
                  </div>
                  ...
                  {appliedCoupon ? (
                    <div className="flex items-center justify-between bg-emerald-50 border border-emerald-300 p-3 rounded-2xl">
                      <div className="flex items-center gap-2.5">
                        ...
                        <div>
                          <span className="font-mono font-black text-xs text-emerald-900 uppercase block">
                            {appliedCoupon.code}
                          </span>
                      ...
                      <button
                        ...
                        className="text-[10px] font-black text-rose-600 bg-white px-2.5 py-1 rounded-xl border border-rose-200 hover:bg-rose-50 transition cursor-pointer shadow-2xs"
                      >
                        Remove
                      </button>
                    </div>
                    ...
                    <div className="space-y-2.5">
                      <div className="flex gap-2">
                        <input
                          ...
                          className="flex-1 border border-stone-200 px-3.5 py-2.5 rounded-2xl text-xs font-black uppercase outline-none bg-stone-50 focus:border-emerald-600"
                          value={couponInput || ''}
                        ...
                        <button
                          ...
                          className="bg-stone-900 hover:bg-stone-800 text-white px-5 py-2.5 rounded-2xl font-black text-xs uppercase cursor-pointer transition shadow-2xs"
                        >
                          Apply
                        </button>
                      </div>
                          ...
                          {availableCoupons.map((coupon) => (
                            ...
                            >
                              <div className="flex items-center gap-2 min-w-0">
                                <div className="w-7 h-7 bg-emerald-100 text-emerald-700 rounded-xl flex items-center justify-center font-black text-[11px] shrink-0">
                                  %
                                </div>
                                <div className="min-w-0">
                                  <span className="font-mono font-black text-[11px] text-stone-900 uppercase block truncate">
                                    {coupon.code}
                                  </span>
              ...
              {!isCartEmpty && (
                <div className="bg-white rounded-3xl p-4 border border-stone-200/80 shadow-2xs space-y-3">
                  <h3 className="font-black text-stone-900 text-xs">Price Details</h3>
                  ...
                  <div className="bg-emerald-50 border border-emerald-200 p-2.5 rounded-2xl text-center text-emerald-800 font-black text-[11px]">
              ...
              {/* CANCELLATION POLICY */}
              {!isCartEmpty && (
                <div className="bg-white rounded-3xl p-4 border border-stone-200/80 shadow-2xs space-y-1">
              ...
              {!isCartEmpty && !session && (
                <div className="p-4 bg-amber-50 border border-amber-200 rounded-3xl text-center space-y-2 shadow-2xs">
                  ...
                  <Link
                    to="/login"
                    onClick={onClose}
                    className="inline-flex items-center gap-1 bg-amber-600 text-white px-5 py-2 rounded-2xl font-black cursor-pointer shadow-xs text-xs"
                  >
            ...
            {!isCartEmpty && (
              <div className="border-t border-stone-200/80 bg-white p-4 space-y-3 shrink-0 shadow-lg">
                <div className="bg-emerald-50 border border-emerald-200/80 px-3.5 py-2 rounded-2xl flex items-center justify-between text-xs">
                ...
                <div className="flex items-center justify-between gap-3">
                  <div className="flex items-center gap-2">
                    <div className="w-8 h-8 bg-emerald-50 text-emerald-700 rounded-xl flex items-center justify-center font-bold border border-emerald-200">
                      <Banknote size={16} />
                    </div>
                    <div>
                      <span className="font-black text-stone-900 block text-xs">Cash on Delivery</span>
                      <span className="text-[10px] text-stone-400 font-bold">Pay when order arrives</span>
                    </div>
                  </div>
                  ...
                  <button
                    ...
                    className="flex-1 bg-amber-400 hover:bg-amber-500 disabled:bg-stone-300 text-stone-950 font-black py-3.5 px-6 rounded-2xl shadow-md transition flex items-center justify-center gap-1.5 uppercase tracking-wider cursor-pointer text-xs"
                  >
