...
export default function ExcelProductUpload({ shopkeeperId, onUploadSuccess }) {
  ...
  const fallbackDummyImages = [
    'https://images.unsplash.com/photo-1543466835-00a7907e9de1?w=500&auto=format&fit=crop&q=80', // Dog
    'https://images.unsplash.com/photo-1514888286974-6c03e2ca1dba?w=500&auto=format&fit=crop&q=80', // Cat
    'https://images.unsplash.com/photo-1537151608828-ea2b11777ee8?w=500&auto=format&fit=crop&q=80', // Puppy
    'https://images.unsplash.com/photo-1573865526739-10659fec78a5?w=500&auto=format&fit=crop&q=80', // Cute cat
    'https://images.unsplash.com/photo-1542838132-92c53300491e?w=500&auto=format&fit=crop&q=80'  // Grocery fallback
  ];

  const getRandomDummyImage = () => {
    const randomIndex = Math.floor(Math.random() * fallbackDummyImages.length);
    return fallbackDummyImages[randomIndex];
  };
    ...
    const templateData = [
      {
        ...
        stock: 100,
        unit: "1 kg",
        image_url: "https://images.unsplash.com/photo-1592924357228-91a4daadcfea?auto=format&fit=crop&w=500&q=80, https://images.unsplash.com/photo-1563536316-33923d83832d?auto=format&fit=crop&w=500&q=80",
    ...
    reader.onload = async (evt) => {
      try {
        ...
        for (let i = 0; i < data.length; i++) {
          ...
          // Parse multiple comma-separated image URLs
          const rawImageUrl = row.image_url || row.Image_Url || row.Image || '';
          let parsedImages = [];
          if (String(rawImageUrl).trim() !== '') {
            parsedImages = String(rawImageUrl).split(',').map(url => url.trim()).filter(Boolean);
          }
          if (parsedImages.length === 0) {
            parsedImages = [getRandomDummyImage()];
          }
          ...
          // 1. Insert parent product with 'approved' status by default & multiple images array support
          const productPayload = {
            shopkeeper_id: shopkeeperId,
            name: String(productName).trim(),
            category_id: categoryId,
            description: String(description),
            image_url: parsedImages[0], // Main display image
            images: parsedImages,       // Array of multiple images gallery
    ...
    <div className="bg-white rounded-3xl border border-stone-200 p-6 space-y-4 shadow-sm font-sans">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pb-4 border-b border-stone-100">
        <div>
          ...
          <p className="text-xs text-stone-500 mt-0.5">Upload products instantly with multi-image links and automatic dummy fallbacks.</p>
        </div>
          ...
          {uploading ? (
            ...
            <>
              ...
              <div>
                ...
                <p className="text-[10px] text-stone-400 mt-0.5">Separate multiple image links with commas in the image_url cell</p>
              </div>
            </>
          )}
        </div>
